...
ALTER TABLE users
  ...
  ADD COLUMN IF NOT EXISTS gender
    ENUM('masculino','feminino','outro','prefiro_nao_informar') NULL AFTER birth_date,
  ADD COLUMN IF NOT EXISTS blood_type
    ENUM('A+','A-','B+','B-','AB+','AB-','O+','O-') NULL AFTER gender,
  ...
  ADD COLUMN IF NOT EXISTS account_status
    ENUM('ativo','inativo','pendente') NOT NULL DEFAULT 'ativo' AFTER job_role;
...
ALTER TABLE users
  MODIFY COLUMN role
    ENUM('paciente','atendente','admin') NOT NULL DEFAULT 'paciente';

SELECT CONCAT(
  COUNT(*), ' usuários migrados: ',
  SUM(role='paciente'), ' pacientes, ',
  SUM(role='atendente'), ' atendentes, ',
  SUM(role='admin'), ' admins.'
) AS resultado FROM users;
